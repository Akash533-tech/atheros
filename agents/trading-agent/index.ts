...
async function runTokenCycle(token: string, agent: NonNullable<Awaited<ReturnType<typeof getAgentFromDB>>>) {
  ...
  // Send live update to dashboard via WebSocket server
  try {
    const eventWithAgent = {
      ...event,
      gas_used: event.gas_used ? event.gas_used.toString() : null,
      agent: { name: agent.name, type: agent.type }
    };
    await axios.post(`${ORCHESTRATOR_URL}/events/live`, eventWithAgent, {
      headers: { Authorization: `Bearer ${API_KEY}` }
    });
  } catch (postErr) {
    logger.warn('Failed to send live event update', { error: String(postErr) });
  }
...
async function runTradingCycle() {
  ...
  for (const token of TRADING_TOKENS) {
    try {
      ...
      const errorEvent = await prisma.agentEvent.create({
        data: {
          agent_id:       agent.id,
      ...
      try {
        ...
        logger.warn('Failed to send live error event update', { error: String(postErr) });
      }
    }
  }
...
async function initializePriceHistories() {
  logger.info('Initializing price histories...');
  const agent = await getAgentFromDB();
  if (!agent) {
    logger.error('No trading agent found, skipping price history initialization.');
    return;
  }

  // Fetch recent events to populate price histories
  ...
  for (const token of TRADING_TOKENS) {
    ...
    try {
      ...
      logger.warn(`Could not fetch current price for ${token} during initialization`, { error: String(err) });
      continue;
    }

    const dbPrices = pastEvents
      .filter((e) => (e.output_json as any)?.token === token)
      .map((e) => (e.inputs_json as any)?.price)
      .filter((p): p is number => typeof p === 'number')
      .reverse();

    if (dbPrices.length > 0) {
      priceHistories[token] = dbPrices.slice(-48);
      logger.info(`Loaded ${priceHistories[token].length} historical prices from DB for ${token}`);
    } else {
      // Generate realistic mock history (random walk)
      let price = currentPrice;
      const mockHist = [];
      for (let i = 0; i < 24; i++) {
        mockHist.push(price);
        // Random walk change between -0.8% and +0.8%
        const change = (Math.random() - 0.5) * 0.016;
        price = price * (1 + change);
      }
      priceHistories[token] = mockHist.reverse();
      logger.info(`Generated random walk price history for ${token} leading to current price $${currentPrice.toFixed(4)}`);
    }
  }
}
...
async function main() {
  ...
  await initializePriceHistories();
