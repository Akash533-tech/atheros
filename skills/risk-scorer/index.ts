...
const MIN_REPUTATION_SCORE = parseInt(process.env.MIN_REPUTATION_SCORE || '30');
...
export function computeRiskScore(params: RiskScorerParams): RiskScore {
  ...
  const score = Math.round(
    ...
    sentimentRisk     * 0.10 +
    reputationRisk    * 0.10 +
    35 // Baseline offset to increase the average risk score
  );
  ...
  const reputationBlock = sourceReputationScore < MIN_REPUTATION_SCORE;
  const approved = finalScore < MAX_RISK_SCORE && !reputationBlock;
  const reason = reputationBlock
    ? `Trade BLOCKED (source reputation ${sourceReputationScore} is below minimum threshold ${MIN_REPUTATION_SCORE}).`
    : buildReason(finalScore, approved, {
