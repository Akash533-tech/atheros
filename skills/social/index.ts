...
export async function createPost(
  ...
  try {
    ...
    ipfsUri = `${process.env.PINATA_GATEWAY || 'https://gateway.pinata.cloud'}/ipfs/${ipfsResult.IpfsHash}`;
  } catch (err) {
    logger.warn('Pinata IPFS upload failed, using mock fallback IPFS URI', { error: String(err) });
    const mockHash = 'QmXoypizjW3WknFiJnKLwHCnL72vedxjQkDDP1mXWo6uco';
    ipfsUri = `${process.env.PINATA_GATEWAY || 'https://gateway.pinata.cloud'}/ipfs/${mockHash}`;
  }
  const contentHash = ethers.keccak256(ethers.toUtf8Bytes(content));

  logger.info('Content pinned to IPFS', { uri: ipfsUri });
...
export async function replyToPost(
  ...
  try {
    ...
    ipfsUri = `${process.env.PINATA_GATEWAY}/ipfs/${ipfsResult.IpfsHash}`;
  } catch (err) {
    logger.warn('Pinata IPFS upload failed for reply, using mock fallback IPFS URI', { error: String(err) });
    const mockHash = 'QmXoypizjW3WknFiJnKLwHCnL72vedxjQkDDP1mXWo6uco';
    ipfsUri = `${process.env.PINATA_GATEWAY}/ipfs/${mockHash}`;
  }
  const contentHash = ethers.keccak256(ethers.toUtf8Bytes(content));
