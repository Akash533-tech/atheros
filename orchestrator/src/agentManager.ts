...
const AGENT_SCRIPTS: Record<string, string> = {
  ...
  'attacker-agent':   path.resolve(__dirname, '../../agents/attacker-agent/index.ts'),
};
...
function spawnAgent(name: string): ChildProcess {
  ...
  // Use cross-platform ts-node binary path directly to avoid shell shim issues
  const isWindows = process.platform === 'win32';
  const tsNodeBin = path.resolve(__dirname, `../../node_modules/.bin/ts-node${isWindows ? '.cmd' : ''}`);
